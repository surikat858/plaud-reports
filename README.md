...
```mermaid
flowchart TD
    A[Запуск скрипта] --> B[plaud today]
    ...
    O -->|Нет| G
    O -->|Да| P[Удаление файлов старше 14 дней]
    P --> F
📦 Требования
...
Нажмите ОК и введите пароль учётной записи.
...
Создайте тестовую запись в приложении Plaud (телефон или компьютер).
...
Запустите скрипт повторно — он должен сообщить, что все записи уже отправлены.
...
Откройте PowerShell Pro Tools.

Выберите "Package" → "Windows Application".

Укажите путь к скрипту и настройте параметры.
...
Создайте пароль для приложения (выберите "Другое" и назовите "Plaud").
...
Репозиторий: github.com/surikat858/plaud-reports
